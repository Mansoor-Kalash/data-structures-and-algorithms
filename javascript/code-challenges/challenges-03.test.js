...
const typeNum = (arr) => {
  let array = arr.filter(value => typeof value === 'number');
  return array;
...
const containsAnd = (arr) => {
  ...
  return arr.filter(el => el.indexOf(and) !== -1);
...
const oddValues = (arr) => {
  let array = arr.filter(value => value % 2 === 1);
  return array;
  // Solution code here...
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let array=[];
  array= arr.filter(value => forbiddenValues.indexOf(value) === -1 );
  return array;
  // Solution code here...
};
